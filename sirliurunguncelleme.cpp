...
#include "ui_sirliurunguncelleme.h"
...
sirliurunguncelleme::sirliurunguncelleme(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::sirliurunguncelleme)
{
    ui->setupUi(this);
...
sirliurunguncelleme::~sirliurunguncelleme()
{
    delete ui;
}

void sirliurunguncelleme::urunCekme()
{
    QSqlDatabase db=QSqlDatabase::addDatabase("QSQLITE");
    db.setDatabaseName("C://Users//reupp//Desktop//Otomasyon//veritabani//database.db");
    db.open();
    QSqlQuery qry(db);
    qry.exec("SELECT rowid,UTuru,UAdi,UBirimFiyat FROM sirliurunler");
    while(qry.next()){
...
void sirliurunguncelleme::tabloTemizle()
{
...
void sirliurunguncelleme::on_uAraTxt_textChanged(const QString &arg1)
{
    QSqlDatabase db=QSqlDatabase::addDatabase("QSQLITE");
    db.setDatabaseName("C://Users//reupp//Desktop//Otomasyon//veritabani//database.db");
    db.open();
    ...
    qry.prepare("SELECT rowid,UTuru,UAdi,UBirimFiyat FROM sirliurunler where UAdi LIKE ?");
...
void sirliurunguncelleme::on_guncelleBtn_clicked()
{
    ...
    fb.open();
    QSqlQuery qry(fb);
    qry.exec("UPDATE sirliurunler SET UTuru='"+uTuru+"',UAdi='"+uAd+"',UBirimFiyat='"+fyt+"' where rowid='"+id+"'");
...
void sirliurunguncelleme::on_tableWidget_clicked(const QModelIndex &index)
{
    int row=ui->tableWidget->currentRow();
...
void sirliurunguncelleme::on_silBtn_clicked()
{
    int row=ui->tableWidget->currentRow();
    QString id=ui->tableWidget->item(row,ID)->text();
    QSqlDatabase fb=QSqlDatabase::addDatabase("QSQLITE");
    ...
    fb.open();
    QSqlQuery qry(fb);
    qry.exec("DELETE FROM sirliurunler where rowid='"+id+"'");
    qDebug()<<qry.lastError();
    fb.close();
